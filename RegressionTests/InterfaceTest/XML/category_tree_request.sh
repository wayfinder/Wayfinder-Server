...
# Make possible to test empty attribute
if [[ $xml_attr_type = "emptytype" ]] {  
   xml_attr_type=""
}

# Replace attribute values in request
./tools/replace.pl "%%%AUTH_USER%%%" $xml_login "%%%AUTH_PASSWD%%%" $xml_passwd "%%%ATTR_CRC%%%" "$xml_attr_crc" "%%%ATTR_LANGUAGE%%%" "$xml_attr_language" "%%%ATTR_TYPE%%%" "$xml_attr_type" < XML/category_tree_request.xml > $request_file
#
